...
`https://crm.globalita.tech`.** Migrada y verificada desde afuera:
...
`finanzas@globalita.tech` sin configurar nada de nuevo. Y se corrigió la
`appURL`, que venía en `http://localhost:8090`: de ahí sale el enlace del
correo de invitación, y sin cambiarlo apuntaba a la máquina de quien lo
recibiera.

La base anterior quedó guardada en el servidor como `data.db.antes-de-migrar`.
